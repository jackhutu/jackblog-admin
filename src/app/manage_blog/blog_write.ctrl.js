...
(function () {
  ...
  angular.module('jackblog.manage')
    ...
    .controller('ModalBlogImagesAddCtrl',function ($scope,$cookies,$timeout,$modalInstance,Upload,Blog,toaster, ServerUrl) {
      //图片种类,初始化为本地上传
      $scope.type = 'local';
      $scope.url = '';
      $scope.uploadImg = function (file, errFiles) {
        ...
        if (file) {
            file.upload = Upload.upload({
                url: ServerUrl + '/article/uploadImage',
                data: {file: file},
            ...
            file.upload.then(function (response) {
              ...
              $modalInstance.close(response.data.img_url);
            }, function (response) {
                if (response.status > 0){
                  var err = response.data.error_msg || '上传图片失败.';
                  toaster.pop('error', '', err);  
                }
            }, function (evt) {
